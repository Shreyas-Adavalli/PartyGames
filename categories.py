CATEGORIES = {
    "Locations": [
        "Airport","Amusement Park","Art Gallery","Bank","Beach",
        "Candy Factory","Casino","Cemetery","Church","Construction Site",
        "Cricket Stadium","Cruise Ship","Dog Park","Embassy","Farm",
        "Gaming Convention","Gas Station","Gold Mine","Hospital","Hotel",
        "Ice Cream Truck","Insane Asylum","Jail","Karaoke Bar","Library",
        "Military Base","Movie Set","Movie Theater","Museum","Noah's Ark",
        "Office Party","Opera House","Pirate Ship","Police Station","Quidditch Field",
        "Race Track","Restaurant","School","Spa Resort","Submarine",
        "Subway","Supermarket","Tram","Trampoline Park","University",
        "Vet","Wedding","X-ray Lab","Yarra River","Zoo"
    ],

    "Celebrities": [
        "Adam Sandler","Ariana Grande","Beyoncé","Billie Eilish","Brad Pitt",
        "Britney Spears","Chris Evans","Chris Hemsworth","Cristiano Ronaldo",
        "Daniel Radcliffe","Donald Trump","Drake","Dwayne Johnson","Elon Musk",
        "Emma Watson","Freddie Mercury","Gordon Ramsey","Harry Styles","Ian McKellen",
        "Jim Carrey","Jennifer Lawrence","Jennifer Anniston","Justin Bieber","Kanye West",
        "Leonardo DiCaprio","Mark Zuckerberg","Messi","Neil Armstrong","Olivia Rodrigo", "Oscar Isaac",
        "Peter Jackson","Princess Diana","Queen Elizabeth II","Rihanna","Robert Downey Jr.","Ryan Reynolds",
        "Scarlett Johansson","Selena Gomez","Shah Rukh Khan","Taylor Swift","Tom Cruise",
        "Tom Holland","Usain Bolt", "Virat Kohli", "Will Smith","Zendaya"
    ],


    "Cartoons": [
        "Adventure Time","Amazing World of Gumball","Avatar: the Last Air Bender",
        "Barney","Ben 10","Ben and Holly","Bluey","Boss Baby","Chota bheem","DogMan",
        "Dora the Explorer","Garfield", "Mickey Mouse Clubhouse","Minions","My Little Pony",
        "Olive the Ostrich","Paw Patrol","Peppa Pig","Penguins of Madagascar",
        "Phineas and Ferb","Regular Show","Scooby Doo","Sesame Street",
        "Shimmer and Shine","Simpsons","Spider-Man","Sponge Bob","Teen Titans Go",
        "Teenage Mutant Ninja Turtles","Tellytubbies","Tom Jerry"
    ],

    "Food": [
        "Apple", "Bagel","Banana","Bread","Burger","Burrito","Cheese","Chicken Nuggets","Chocolate Cake",
        "Cupcake","Curry","Donut","Dosa","Dumplings","Eggs","French Fries","Fried Chicken","Grilled Cheese",
        "Garlic Bread","Hot Dog","Ice Cream","Idli","Juice","Kebab","Korean BBQ","Lasagana","Mango","Milk","Muesli Bar",
        "Nachos","Orange","Pancakes","Pasta","Pie","Pizza","Popcorn","Quesadilla","Ramen","Salad",
        "Sandwich","Smoothie","Soda","Soup","Steak","Sushi","Tacos","Upma","Vodka","Waffles","Watermelon","Yogurt","Zucchini"
    ],


    "Animals": [
        "Ant","Badger","Bear","Cat","Chameleon","Crocodile","Deer","Dog","Dolphin",
        "Eagle","Elephant","Fish","Fox","Frog","Giraffe","Hippo","Horse","Immortal Jellyfish",
        "Jaguar","Kangaroo","Lion","Meerkat","Monkey","Narwhal","Octopus","Otter","Owl","Panda",
        "Parrot","Penguin","Quail","Rabbit","Raccoon","Shark","Sloth","Squirrel","Tiger",
        "Turtle","Urchin","Vampire Bat","Wolf","Yak","Zebra"
    ],


    "Countries": [
        "Argentina","Australia","Bangladesh","Belgium","Brazil","Canada","Chile",
        "China","Colombia","Egypt","France","Germany","Greece","India","Indonesia",
        "Ireland","Italy","Jamaica","Japan","Kenya","Malaysia","Mexico","Nepal",
        "Netherlands","New Zealand","Nigeria","Norway","Pakistan","Papua New Guinea",
        "Peru","Philippines","Portugal","Qatar","Russia","Saudi Arabia","Singapore",
        "South Africa","Spain","Sri Lanka","Sweden","Switzerland","Thailand",
        "Turkey","United Kingdom","USA","Vietnam"
    ],
}